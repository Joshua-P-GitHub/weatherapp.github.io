...
export default function Weather() {
      ...
      if (response.data.length > 0) {
        ...
        if (state !== ' ') {
          ...
          if (!foundOne) {
            ...
            stateName = response.data[0].state ? response.data[0].state : ""
          }
        }
  ...
  const [inputClick , toggleInputClick] = useState(false)
  ...
  return (
    <div id="weather">
      <header>
        <NavBar />
        <input className={`full-input ${inputClick && 'glass'}`} placeholder="Enter City" onKeyDown={handleSubmit} onFocus={() => {
          toggleInputClick(true)
        }} onBlur={() => {
          toggleInputClick(false)
        }}/>
      </header>
      <main>
        <div className="card-holder">
          {Object.values(places).map((place) => {
            ...
            return <WeatherCard
              key={`${place.cityName.replace(/ /g, '-')}${place.stateName.replace(/ /g, '-')}`}

              name={`${place.cityName}, ${place.stateName}`} backGround={weather === 'Clear' ? sunnyBG : weather === 'Clouds' ? cloudyBG : weather === 'Rain' ? rainyBG : weather === 'Mist' ? mistBG : sunnyBG}

              temp={place.main} weather={place.weather[0]}

              delete={handleDelete}

              keyName={`${place.cityName.replace(/ /g, '-')}${place.stateName.replace(/ /g, '-')}`}
